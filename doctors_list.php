...
if($action === 'list'){
    ...
    $mysqli->query("UPDATE users SET status = 'active' WHERE (status IS NULL OR status = '') AND role='user'");
    
    $res = $mysqli->query("SELECT id, name, email, mobile, role, COALESCE(status, 'active') as status, created_at FROM users WHERE role='user' ORDER BY id DESC");
    $out = [];
