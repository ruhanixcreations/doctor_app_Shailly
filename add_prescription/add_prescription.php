...
/*
| Save draft prescription to database
| This stores temporary prescription data that's being worked on
*/
if($action === 'save_draft'){
    ...
    $patient_id = trim((string)$data['patient_id']);
    $symptoms = isset($data['symptoms']) ? $data['symptoms'] : '';
    $medicineEntries = isset($data['medicineEntries']) ? json_encode($data['medicineEntries']) : '[]';
    $selectedBloodTests = isset($data['selectedBloodTests']) ? json_encode($data['selectedBloodTests']) : '[]';
    $followUpDate = isset($data['followUpDate']) ? $data['followUpDate'] : null;
    
    // Create draft table if it doesn't exist
    $mysqli->query("CREATE TABLE IF NOT EXISTS prescription_drafts (
        id INT AUTO_INCREMENT PRIMARY KEY,
        patient_id VARCHAR(64) NOT NULL,
        symptoms TEXT,
        medicine_entries TEXT,
        selected_blood_tests TEXT,
        follow_up_date DATE,
    ...
    $stmt = $mysqli->prepare("INSERT INTO prescription_drafts (patient_id, symptoms, medicine_entries, selected_blood_tests, follow_up_date) VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE symptoms=VALUES(symptoms), medicine_entries=VALUES(medicine_entries), selected_blood_tests=VALUES(selected_blood_tests), follow_up_date=VALUES(follow_up_date)");
    if(!$stmt) {
        echo json_encode(['success'=>false,'message'=>'Prepare failed: '.$mysqli->error]); exit;
    }
    
    $stmt->bind_param('sssss', $patient_id, $symptoms, $medicineEntries, $selectedBloodTests, $followUpDate);
    if($stmt->execute()) {
...
/*
| Load draft prescription from database
*/
if($action === 'load_draft'){
    $patient_id = $_GET['patient_id'] ?? '';
    if(!$patient_id){ echo json_encode(['success'=>false,'message'=>'patient_id required']); exit; }
    
    $stmt = $mysqli->prepare("SELECT symptoms, medicine_entries, selected_blood_tests, follow_up_date FROM prescription_drafts WHERE patient_id = ? LIMIT 1");
    if(!$stmt){
    ...
    if($row){
        $draft = [
            'symptoms' => $row['symptoms'],
            'medicineEntries' => json_decode($row['medicine_entries'], true) ?: [],
            'selectedBloodTests' => json_decode($row['selected_blood_tests'], true) ?: [],
...
/*
| Clear draft prescription from database
*/
if($action === 'clear_draft'){
    ...
    $patient_id = trim((string)$data['patient_id']);
    $stmt = $mysqli->prepare("DELETE FROM prescription_drafts WHERE patient_id = ?");
    if(!$stmt) {
        echo json_encode(['success'=>false,'message'=>'Prepare failed']); exit;
    }
